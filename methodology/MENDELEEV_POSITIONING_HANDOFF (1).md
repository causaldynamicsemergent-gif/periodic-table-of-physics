...
**What E0 does — the lead move plus five tile-chrome refinements, none of which change the data layer:**

**E0 lead — open-frontiers-as-primary-surface (parallel-views architecture).** Per `MENDELEEV_FRAME.md` §7, the decisive E0 move is not tile chrome — it is making the open frontiers a *primary surface* rather than a sidebar afterthought. The explorer ships two parallel top-level views with a toolbar toggle:

- **Open-questions view (default landing).** The eleven open-frontier nodes render as primary cards, grouped by the structural reason the current theory breaks down there (cross-architecture / foundational frontiers first, per §6's M1-leads ordering). Each card carries the empirical bound any resolution must satisfy (≳ / ≲ / ≈), the structural reason, the named candidate resolutions with their current viability, a one-line teaser of what is *structurally forced* if a resolution holds, and — per the focus-not-filter discipline — the frontier's structural neighbourhood (the classifications that bear on it) as pills that walk directly into those classifications. Clicking a card opens the full frontier detail in the sidebar, reusing the existing `renderFrontierCard` (carrier bound, the if-real-implies tree, candidate-targeting, experimental coverage). This is the M1 empty-cell-prediction move made the *first thing a physicist sees*.
- **Structural-classifications view.** The existing formal-classification tile grid, unchanged in substance. The five tile-chrome moves below (E0a–E0e) are refinements *on this view*, not the whole of E0.
- **Comparison panel (M1 mode 2) and configuration builder (M1 mode 3)** are subsequent sub-PRs, not part of the E0 lead; `MENDELEEV_FRAME.md` §7 sequences them after the parallel-views architecture lands.

The default landing is the open-questions view; the toggle persists the physicist's choice (localStorage); an FC- or edge-selection deep-link opens the classifications view so existing bookmarks are honoured.

**The five tile-chrome refinements (on the structural-classifications view):**
...
> **Shipped-variant note (E0d).** In the E0-lead implementation the Mendeleev frame sentence leads the *open-questions view* as its intro lede, where it has the most context; the masthead retains its dataset-counts line (set by `init()` in `explorer-sidebar.js`). A fuller masthead-subtitle treatment per the E0d draft above remains an optional, trivial follow-up in `explorer-sidebar.js`.
...
P0 (draft MENDELEEV_FRAME.md)
  ...
  │     └─ E0-lead (parallel-views: open-questions landing) → E0a–E0e (tile-chrome on the classifications view)
...
P0 first. E0 and M0 can land in parallel after P0 ships. Within E0 the **E0 lead** (parallel-views architecture, open-questions landing) lands first; E0a–E0e are tile-chrome refinements on the structural-classifications view and follow it. E1–E8 sequence picks up after E0 (the existing handoff's queue rewrites to start with E1 unchanged, but now landing on an E0-Mendeleev-framed surface). The use-side artifacts queue sequences after E1–E5 close as already specified in `TRACK_4_USE_SIDE_ARTIFACTS.md` §3.
...
### E0 lead — Open-frontiers-as-primary-surface (parallel-views architecture)
- New module `explorer-frontiers.js` + `update-e0.css`; touches `explorer-data.js` (view state + hash routing), `explorer-map.js` (`renderMap` view dispatch + toggle wiring + `zoomFitToView` guard), `Map_v34_explorer.html` (toolbar toggle, script/style tags).
- Open-questions view is the default landing; toolbar toggle switches to the structural-classifications (tile-grid) view; the choice persists across reloads (localStorage); FC/edge deep-links open the classifications view so existing bookmarks are honoured.
- The eleven open-frontier cards carry the empirical bound (≳/≲/≈), the structural reason, candidate resolutions with viability, the forced-consequences teaser, and the focus-not-filter structural neighbourhood (clickable into the constraining classifications); full detail reuses `renderFrontierCard`.
- Reuses the shipped E2–E5 renderers (`renderQSCallout`, `renderIfRealImpliesSection`, `renderTargetedByTarget`) for the sidebar detail rather than duplicating them.
- Out of scope (subsequent sub-PRs): comparison panel (M1 mode 2), configuration builder (M1 mode 3), fuller E0d masthead subtitle.
- One chat. Ships before E0a–E0e.
...
- **The literature-anchoring principle binds the positioning prose as it binds the data layer.** A 2026-05-28 maintainer call sharpened the firewall: substrate content is added only when the indexing / synthesis is itself proposed in the literature — not when the project would be synthesizing across literature-anchored fragments. The principle implies a structural distinction the positioning prose must carry: substrate-contained moves (M1, M4) and substrate-enabled moves (M2, M3) are different categories. M1 / M4 surfaces describe what the map directly holds. M2 / M3 surfaces describe what the substrate enables the physicist to do, with the discovery / adjudication work explicitly framed as the user's. Surfaces that frame M2 / M3 as "the map's claims about cross-classification patterns and unification adjudications" overstate the project's authority and re-introduce the project-synthesis failure mode the firewall prohibits. The frame doc, the subtitle, the tool descriptions, and the glossary entries all carry this distinction.
