...
The live MCP server: `https://map-of-physics.eddie-8e5.workers.dev` — serving v40 / v17 / Phase B, 29 tools (since Phase B Step N+1 deployment). Will be rebuilt against v18 in Phase C's final step. Note: the MCP server still serves v40 even though the canonical data is now v41; the rebuild will catch up the server to the latest data + schema.
The canonical data: `data/Map_v34_consolidated.json`, internal version `v41` (since Step 0.5 merge). Counts: 83 nodes, 191 edges, 30 formal-classifications, 484 cells, 22 glossary entries, 19 experimental programs, 102 cross-classification edges. Phase B added 23 `if_real_implies` resolutions on 14 of 17 carriers, 24 implications total. Phase C Step 0.5 added 12 forward-looking experimental-program nodes (DUNE, Hyper-K, JUNO, GW ground network, LISA, CMB-S4, Rubin/LSST, DESI, axion haloscopes, EDM program, muon g-2 continuation, FCC) and bumped v40 → v41.
...
3. Tell it which step the work resumes at — e.g., *"Pick up at Step 1 from PREDICTIVE_LAYER_PHASE_C_HANDOFF.md: the v18 schema-spec extension. Start by fetching MAP_v17_schema_spec_extension.md as the template, then draft against the Phase C scope memo §2-§5. The v41 dataset now has real experimental-program ids (dune, hyper-k, juno, gw-ground-network, lisa, cmb-s4, rubin-lsst, desi, axion-haloscope-network, edm-program, muon-g-2-continuation, fcc) that can ground the §6 examples."*
4. The chat fetches code, data, and schema from GitHub via `web_fetch`. Raw URLs in the Quick reference below.

The Map of Physics MCP tools are usually deferred behind `tool_search` in fresh chats. If the assistant doesn't see them, ask it to call `tool_search` with keyword `"map of physics"`. **Important:** the connector still serves v40 / v17 / Phase B (29 tools) until the Phase C Step N+1 rebuild lands; v41 nodes added in Step 0.5 are not yet queryable through MCP. Inspect them via `web_fetch` on the raw `Map_v34_consolidated.json` URL until the rebuild.
...
- MCP server: `https://map-of-physics.eddie-8e5.workers.dev` (v40 / v17 / Phase B, 29 tools — note: lags canonical data which is v41 since Step 0.5)
...
- v41 data (1.5 MB — exceeds github.com blob fallback size limit, use raw CDN or MCP): `https://raw.githubusercontent.com/causaldynamicsemergent-gif/periodic-table-of-physics/main/data/Map_v34_consolidated.json`
...
*End of PREDICTIVE_LAYER_PHASE_C_HANDOFF.md, v2 (Steps 0 + 0.5 shipped — scope memo settles the six shape decisions; 12 forward-looking experimental-program nodes added to v41, closing the source-end-node prerequisite for Phase C Step 4.4. Step 1 (v18 schema-spec extension) is next, modelled on Phase B's authoring pattern). Update after every shipped step.*
