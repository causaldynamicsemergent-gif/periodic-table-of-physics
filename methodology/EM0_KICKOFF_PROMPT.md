...
```
Pick up sub-PR EM0 from MENDELEEV_POSITIONING_HANDOFF.md §3.1: surface the
...
Background. The frame doc (MENDELEEV_FRAME.md) has shipped (P0''). EM0 makes
...
already read as Mendeleev-style, not as a review-article catalog. EM0 changes
...
Scope — the five EM0 moves (MENDELEEV_POSITIONING_HANDOFF.md §3.1):
  EM0a. In-tile constructive grid: realized / conjectured-by-pattern /
       forbidden-by-pattern visually distinct from each other AND from
       not-yet-authored cells (three fills, one absence).
  EM0b. Conjectured-by-pattern cells draw the eye (hatched fill / saturated
  ...
  EM0c. At least one cross-classification recurrence visible by default,
  ...
  EM0d. A one-sentence subtitle under the title, in the Mendeleev frame.
  EM0e. Per-tile closure indicator rendered from the closure_level field.
...
1. §0 state verification. Call server_info; confirm data_version,
   ...
   still serving Phase A with no Mendeleev subtitle (the baseline EM0
   ...
   and stop — EM0 has nothing to source from).
...
3. Read, then quote one load-bearing sentence back from each of:
   ...
   - META_v21_1_methodology_firewall.md §8 (content / organization
     distinction);
   - MENDELEEV_POSITIONING_HANDOFF.md §3.1 (the five EM0 moves) and §7
     (corrected literature-anchoring principle).
   The quote-back is operational, not ceremonial.

4. Read PHYSICIST_FACING_VOCABULARY.md §3 in full. ALL EM0 surface text
...
5. Verify each move EM0 renders is backed by current data, with one
   representative query each (record what returns):
   - EM0a/EM0b forbidden-by-pattern: find_forced_cells / find_structurally_
   ...
   - EM0c cross-classification: find_cross_classification (status filter) —
     name one recurrence the default-on indication can show.
   - EM0e closure: get_classification on two FCs; confirm closure_level is
...
EM0d subtitle, all four moves (M1 empty-cell prediction, M2 cross-
...
You draft the subtitle; the maintainer approves it (per §3.1 EM0d). Same for
the EM0a/EM0b palette (colour-blind-safe), the EM0c arrows-vs-chips choice, and
the EM0e badge form — these are the §3.1 design questions; resolve them in
...
Then propose the EM0 PR shape for maintainer review BEFORE writing code:
...
**2. The P0'' commit-state checks survive as a lightweight re-verification.** Step 1 has the next session re-confirm that `MENDELEEV_FRAME.md` is committed and the firewall has a §8, rather than assuming. Both are now true; the check is cheap insurance against a future re-drift, and it tells the EM0 chat to stop if its source text is missing.

**3. Surface text is sourced from the frame doc, not from memory.** EM0's whole job is to make the frame doc's moves visible, so the prompt points the implementing chat at the §1/§2/§6 physicist-facing paragraphs as the canonical language and binds all surface text to `PHYSICIST_FACING_VOCABULARY.md` §3. The subtitle candidate is a starting draft seeded from the frame doc, explicitly left for the implementing chat to refine and the maintainer to approve (per §3.1 EM0d's "implementing chat drafts; maintainer approves").

**4. Move-verification queries are EM0-specific.** The data-verification step targets exactly what EM0 renders: the 22 forbidden-by-pattern cells and the zero conjectured-by-pattern count (EM0a/EM0b), a named cross-classification recurrence (EM0c), and `closure_level` presence (EM0e). This keeps the "verify the move against current data" discipline from the P0 prompts, retargeted to the surfacing work.
...
- **M0 needs its own kickoff prompt.** The "After P0'' ships" note in `P0_PRIME_PRIME_KICKOFF_PROMPT.md` pairs EM0 and M0; M0 is independent of EM0 and lands on the MCP worker side (`server_info` purpose field, tool-description rewrites, tool grouping, glossary entries — `MENDELEEV_POSITIONING_HANDOFF.md` §3.2). Its kickoff prompt applies the same §0 template, sources its surface text from the same frame doc, and carries the same standing emphasis note. Draft it against the live worker state when M0 starts.
- **§6 bookkeeping.** When EM0 ships, close the EM0 row in `MENDELEEV_POSITIONING_HANDOFF.md` §6 and verify the explorer's first-impression surface is Mendeleev-framed (at least one constructive prediction and at least one cross-classification recurrence visible without interaction, per `MENDELEEV_FRAME.md` §7).
- **Then E1.** The existing `EXPLORER_PHASE_BC_HANDOFF.md` E1–E8 queue resumes, now landing on an EM0-Mendeleev-framed surface; after E1–E5, the `TRACK_4_USE_SIDE_ARTIFACTS.md` use-side queue follows.

---

*End of EM0_KICKOFF_PROMPT.md, drafted 2026-05-30 at the close of the §0-verification-clean session that confirmed the two P0'' deliverables landed. Paste the prompt block into a fresh Claude chat to execute EM0 (explorer Mendeleev surfacing) once the §0 verification in `CONTINUATION_HANDOFF_AFTER_P0PP.md` passes. Per project discipline: the live system + canonical data are authoritative, not this document — verify before relying.*
