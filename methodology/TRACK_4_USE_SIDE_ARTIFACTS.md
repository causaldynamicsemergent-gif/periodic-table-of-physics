...
**STATUS: SHIPPED (2026-05-30) → [`/use/essay.md`](../use/essay.md).** ~1,970 words, three worked examples in physicist prose: leptogenesis (the frontier-level forced-prediction move — the Mendeleev claim kept frontier-level per the standing flag), the strong-CP problem (two-channel experimental coverage + the informative-empty discrimination result), and the catalogue's seventeen falsified predictions (the negative-space move + the Type-A/Type-B regularity that only BSM-overreach and over-claimed signals get falsified). All three verified against the live catalogue at data v99 / schema v20; written in the vocabulary discipline. The falsified-prediction example is carried inline in the essay rather than as a separate session file (see §2).
...
**STATUS: SHIPPED (2026-05-30) → [`/use/recipes.md`](../use/recipes.md).** The "How to use the map" page now aggregates the distilled-recipe paragraphs from the worked sessions in `/use/sessions/` (leptogenesis, strong-CP), written in the vocabulary discipline. It currently carries the two sessions' distilled-recipe paragraphs (reading an open problem for what it forces; reading a frontier's experimental coverage) plus a closing note on informative emptiness, and grows as new sessions land. Lives in `/use/`, not the explorer, per the §4 placement decision below.
...
**STATUS: SHIPPED — first three of the planned set (2026-05-30).** Three edited worked sessions are in [`/use/sessions/`](../use/sessions/): [`leptogenesis.md`](../use/sessions/leptogenesis.md) (frontier-level Mendeleev prediction move, verified against the live `matter-antimatter-asymmetry` node), [`strong-cp.md`](../use/sessions/strong-cp.md) (two-channel resolver + the informative-empty `find_discriminating_experiments` result, verified against the live `strong-cp-problem` node), and [`mass-ordering.md`](../use/sessions/mass-ordering.md) (convergent coverage of a single fixed quantity via three orthogonal channels, verified against the live `cell-nu-mass-hierarchy` cell and the `hyper-k` / `dune` / `juno` resolves edges). All three follow the recipe → session → "what this demonstrates" → distilled-recipe structure and the vocabulary discipline. The edited-transcript tier was chosen (see §4). More sessions may come; the empty discriminating-experiments result is realized twice over — in `strong-cp.md` (empty because candidates supply ranges) and in `mass-ordering.md` (empty because the quantity is a fact of nature with no competing predictions), which together teach the two distinct causes of informative emptiness.
...
**Example A — leptogenesis → Davidson-Ibarra → M₁ ≳ 10⁹ GeV.** The matter-antimatter-asymmetry frontier carries an `if_real_implies` leptogenesis resolution. *(Correction, 2026-05-30: verified against the live node, the resolution forces **two** `promotes_subtype` consequences, not the four originally sketched here.)* The Dirac-vs-Majorana cell in neutrino-sector-phenomenology (presently predicted-only) is forced to resolve to Majorana, carrying a quantitative lower bound on the lightest right-handed neutrino, M₁ ≳ 10⁹ GeV (Davidson-Ibarra 2002); and the leptogenesis cell is promoted from predicted-only to confirmed-by-indirect-evidence. The example demonstrates: a frontier resolution pins specific cells to specific values in a stated direction; quantitative bounds attach to specific implications with named citations; "if real, this is forced" is a structural query the dataset answers and a vanilla LLM cannot. Shipped as [`/use/sessions/leptogenesis.md`](../use/sessions/leptogenesis.md) and as the first essay example.

**Example B — HK / DUNE / JUNO three-channel mass-ordering redundancy.** *(Shipped 2026-05-30 as [`/use/sessions/mass-ordering.md`](../use/sessions/mass-ordering.md), verified against the live `cell-nu-mass-hierarchy` cell and its three resolves edges.)* A natural physicist question — "which experiments will settle the neutrino mass ordering?" — returns three programs with three independent physical channels: matter-driven beam (DUNE, ≥5σ for all δ_CP after 2 yr at the 1285-km baseline, Abi et al. 2020), statistical-power atmospheric (HK, ≥3σ from 10-yr atmospheric alone rising to ≥5σ with beam + favorable θ₂₃ octant, Design Report 2018), vacuum-interference reactor (JUNO, 3σ median at ~6.5 yr × 26.6 GW, 52.5 km, Abusleme et al. 2025). The example also demonstrates *informative emptiness*: `find_discriminating_experiments` returns empty for all three pairings because none of these programs predict competing point values — the ordering is the sign of one mass-squared difference, a fact nature has already fixed, and the three programs all measure that same fact through orthogonal channels rather than adjudicating rival predictions. The curators encoded this by leaving the per-program prediction lists empty; all three edges are marked bounds-setting. The catalogue distinguishes "experiments that adjudicate between competing predictions" from "experiments that measure a shared quantity through orthogonal channels"; a vanilla LLM conflates the two. The session pairs with `strong-cp.md` to teach the two distinct causes of an empty discriminating-experiments result (range-valued candidates there; a fixed fact of nature here).
...
**Example E — the falsified-prediction structure (USED in essay §1.1).** The catalogue records **seventeen** falsified predictions *(correction, 2026-05-30: the v21 META doc §3 O5 says "three" — that was the v21 snapshot; live data at v99 has seventeen, and the regularity O5 noted holds across all of them)*. They fall into two recorded kinds the dataset itself tags: **Type-A**, a specific BSM hypothesis ruled out (minimal SU(5) proton decay via Super-K τ > 2.4 × 10³⁴ yr; MACHOs < 8% of the halo via EROS-2; pure MOND at clusters via the Bullet Cluster; self-accelerating DGP; generic TeVeS / Horndeski via GW170817's c_T bound; natural-parameter Brans-Dicke via Cassini; the deeply-bound H-dibaryon via lattice QCD; PBH-as-100%-DM in the LIGO window via merger-rate + microlensing bounds), and **Type-B**, an over-claimed empirical signal excluded by more data (CDMS-Si 3-event WIMP hint; EDGES 21cm; DAMA/LIBRA's WIMP interpretation; Heidelberg-Moscow 0νββ; the LSND sterile interpretation; the retracted 2e²/h Majorana-conductance claim). The example demonstrates: the catalogue records its negative space, not just confirmations; a structural regularity (settled physics never enters the falsified bucket — only ambition that outran evidence); and curatorial care at the boundary (the DAMA modulation is real at 12.9σ even though its WIMP reading is dead; the Majorana hypothesis is open even though the Heidelberg-Moscow positive claim is gone). Carried inline in the essay rather than as a standalone session.

A further example slot remains open. With Example B now shipped, the strongest remaining candidate is the **doubly-orphaned ADE clique** (a structural observation showing the dataset records its own incompleteness — distinct from the predictive and coverage moves the three shipped sessions cover).
