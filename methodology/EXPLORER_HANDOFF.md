...
**Currently deployed on the live site: Update A.1 + Update B + Update C glossary panel + Update C clickable on-map edges (seven-file build).** Any chat that ships a new build is responsible for updating this line in the same commit.
...
- **Update C — remaining.** Three items remaining after clickable on-map edges shipped, each independent and shippable in its own chat. **Next up: typography pass.** Two parts: (1) KaTeX inline math — the dataset uses `$...$` and `$$...$$` notation in cell descriptions, edge-card descriptions, and many other prose fields, and these currently render as raw markup; (2) a scholarly serif body face — Crimson Pro is loaded but underused, body text is mostly Spectral or fallback. After typography: in-tile cell visualisation (mini-grids of cell swatches inside tiles with under ~20 cells), and discourse-highlight ring badges (small clickable corner-of-tile element on FCs lit up by a selected discourse node, opening a "why is this FC connected" card that reuses the xc-edge-card chrome from clickable edges). Sketches in the "Update C remaining" section below.
- **Step 5 — open the review pathway** (now further unblocked — Update B made the discourse layer navigable, the Update C glossary lowers the explanation overhead for an unfamiliar physicist reviewer encountering map-internal vocabulary like "discharge status" or "candidate-foundational," and Update C clickable edges surfaces previously-buried citation chains that subfield specialists will want to verify against their literature). GitHub Issues templates for cell errata / new-FC proposals / cross-class edge proposals; `/methodology/EDITORIAL.md` describing the editorial process; outreach copy + the live explorer link sent to 2–3 working physicists in different subfields. The "Step 5" sketch later in this doc has the operational details.
...
3. Tell it which section of this doc the work resumes at — e.g., *"Pick up at Update C remaining as described in EXPLORER_HANDOFF.md, starting with the typography pass."*
...
**explorer-map.js** (~19 KB) — periodic-table renderer (`renderMap`, `renderTile`), hover tooltip (`showTileTooltip`, `moveTooltip`, `hideTooltip`), toolbar wiring (`wireToolbar`, `syncToolbarChips`, `wireRowsByDropdown`), zoom + pan: `ZOOM_MIN = 0.35`, `ZOOM_MAX = 2.0`, plus `applyZoom`, `clampPan` (80 px MARGIN, depends on `wrap.scrollHeight * state.zoom`), `zoomFitToView`, `zoomIn`, `zoomOut`, `wireMapDragPan`. **Update B added:** `ROWSBY_OPTIONS` constant; multi-select spotlight logic in `renderTile` (Set-based dim/spot instead of single-value); discourse-highlight class on `renderTile` driven by `isFCConnectedToSelectedDiscourse`; spotlight button count and discourse-tab button active-state in `syncToolbarChips`; rows-by chip group replaced with button + dropdown menu; spotlight chip group replaced with button that opens the sidebar panel. **Untouched by both Update C passes.**
...
**Map_v34_explorer.html** (~72 KB) — `<head>` + ~50 KB of CSS (A.1 styles + folded `update-b.css` for discourse cards, spotlight panel, rows-by dropdown, peer-button toolbar group, discourse-highlight tile ring, sidebar classifications panel + folded `update-c.css` glossary styles: `.glossary-entry`, `.glossary-field`, `.glossary-term`, `.glossary-related-pill`, etc. + folded `update-c-edges.css` for clickable edges: SVG `z-index: 5`, `.phen-phen-hit` and `.phen-phen-edge` and `g.phen-phen-edge-group:hover` rules, `.dc-edge-clickable` hover/focus styles, `.xc-edge-card` chrome with endpoints/meta/targeted-by/citation-list styling) + body HTML (masthead, toolbar with peer catalogue buttons, body-grid with map-pane + splitter + sidebar including the **four-button quick-bar** [§ Glossary added between Phenomena and About], tooltip, help-overlay, toast), then the seven script tags. No inline JS apart from the one-line `init()` call. **7 KB over the 65 KB soft target** — the typography pass will add significant CSS (KaTeX styles, font-face declarations, body-face overrides), so it should ALSO extract `update-b.css` and `update-c.css` and `update-c-edges.css` into separate `/explorer/*.css` files loaded via `<link>` tags before the script tags. Doing the extraction in the same commit as typography prevents the HTML from drifting further.
...
| Restyle the edge card visuals (endpoints, pills, citations) | `Map_v34_explorer.html` (CSS `.xc-edge-card`, `.xc-edge-endpoints`, `.xc-edge-meta`, `.xc-cells-grp`, `.xc-citation-list`) |
| Tune SVG line hit-area width or arrow target size | `Map_v34_explorer.html` (CSS `.phen-phen-hit` stroke-width) + `explorer-phenomena.js` (arrow-hit triangle geometry constants `wHit`, `ahxHit`, `ahyHit`) |
| Recolor or restyle the visible phen↔phen line | `Map_v34_explorer.html` (CSS `.pt-overlay-svg path.phen-phen-edge`) |
...
Three items remaining after clickable on-map edges shipped. Each is independent and can ship in its own chat. Sketches follow.

### Typography pass (next up)

Two parts: KaTeX inline math + scholarly serif body face.

**KaTeX bundling.** The v34 dataset uses `$...$` and `$$...$$` notation in cell descriptions, edge-card descriptions, FC descriptions, predictions, and glossary entries. Currently renders as raw markup. Two bundling strategies:

- **CDN at runtime.** `<script src="https://cdn.jsdelivr.net/npm/katex@0.16.x/dist/katex.min.js">` + `<link rel="stylesheet" href="https://cdn.jsdelivr.net/npm/katex@0.16.x/dist/katex.min.css">` in the HTML skeleton. ~280 KB total over the network; cached by browsers after first load. Call `katex.render()` (or `katex.renderToString()`) from within `formatPara`. No build pipeline change — fits the no-local-terminal maintainer pattern. Risk: CDN availability + a new external dependency in the load path.
- **Server-side render at augment time.** Add KaTeX as a Node-side dependency, pre-render math expressions in `augmentDataset` to inline HTML/SVG, ship the rendered HTML. Zero runtime cost. Adds a regenerate step when data changes — the maintainer's no-local-terminal pattern would need a CI hook or a manual `npm run regenerate` step on PRs touching `Map_v34_consolidated.json`.

Lean: CDN. Fits the existing operational pattern. Confirm or override in the design-proposal step.

**Math-render scope.** Two helper-placement options:

- `formatPara` becomes math-aware. Every string that passes through `formatPara` gets KaTeX. Cleanest, but also applies to cell titles, axis labels, and other places where math is rare or absent.
- New `formatProseWithMath` helper, called explicitly from the renderers that pass through long prose (cell description, edge description, glossary entry definition, FC description). More surgical, requires touching every relevant call site.

Lean: extend `formatPara`. The cost of running KaTeX over math-free strings is negligible (a regex match that fails fast).

**Body face.** Crimson Pro is loaded in the HTML's `<link>` to Google Fonts but underused — body text is mostly Spectral or fallback (Georgia / serif). Pick one consistent body face and apply it to descriptions, card bodies, glossary entries. Crimson Pro is the better choice for math-heavy prose (taller x-height, more consistent baseline with KaTeX's CMU output) — but the call should be made by the typography-pass implementer with the live page in front of them.

### In-tile cell visualisation

For classifications with under ~20 cells, render a mini-grid of cell swatches inside the tile preview (rather than just the symbol + counts). Makes inner structure visible at a glance. Tiles with more than ~20 cells continue to show just the count. Implementation: branch in `renderTile` (`explorer-map.js`) by `fc.cell_count`; the mini-grid uses CSS grid with small swatches colored by content-type (empty / has-predictions / structurally-excluded / has-realized-examples).

### Discourse-highlight ring badges

Deferred from the clickable-edges pass. A small clickable corner-of-tile element on FCs lit up by a discourse-node selection, opening a card that explains why this FC connects to the selected discourse node. Reuses the xc-edge-card chrome (header pills + description + structured sections); the trigger surface (corner badge) and card content (discourse-edge details) are the new parts.

### Update C operational expectations

Per the "Edits, not regenerations" section, each Update C item is one chat, one to three files touched, well under 30K tokens output. The smoke test extends by 5–10 assertions per item. Floor is now 91 (raised by clickable edges from the prior 80). Ship each item independently; don't bundle.
...
Each module targets ≤ 60 KB. HTML skeleton ≤ 65 KB (currently **~72 KB after Update C clickable on-map edges — 7 KB over the soft target**; the typography pass should ALSO extract `update-b.css`, `update-c.css`, and `update-c-edges.css` into their own files in `/explorer/`, loaded via `<link>` tags before the script tags, since typography will itself add significant CSS — KaTeX styles, font-face declarations, body-face overrides). If a single file approaches the cap, split it further. The monolith broke down at ~150 KB; modules feel comfortable up to ~60 KB.
...
- **The 91-assertion smoke-test floor.** Update B landed at 80, Update C glossary added 46 against a glossary-specific stub (narrower scope), Update C clickable edges landed at 91 against a comprehensive stub. **The floor for future passes is 91**, raised by clickable edges. Future passes should extend by 5–10 assertions per item to stay safely above.
