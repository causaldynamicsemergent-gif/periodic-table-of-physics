...
The data layer is stable; the active work was presentation. The build has completed Update C in full — all five items shipped (glossary panel, clickable on-map edges, typography pass, in-tile cell viz, discourse-highlight ring badges). The next milestone is Step 5 (open the review pathway), which is text-and-process work rather than code.

**Currently deployed on the live site: Update A.1 + Update B + Update C glossary panel + Update C clickable on-map edges + Update C typography pass + Update C in-tile cell viz + Update C discourse-highlight ring badges (eleven-file build: 1 HTML + 4 CSS + 6 JS).** Any chat that ships a new build is responsible for updating this line in the same commit.
...
- **Update C — discourse-highlight ring badges.** When a discourse node is selected, the existing amber discourse-highlight ring already lit up every FC connected to it; the badge adds a clickable amber chip in the top-right corner of each lit-up tile (above the falsified-flag's row, shifted to clear it via a `.pt-tile.discourse-highlight .fals-flag { margin-right: 28px }` nudge). The badge icon mirrors the *type* of the selected discourse node (⌬ architecture, ✕ frontier, ◇ totality, ▣ regime, ⚙ program) using the existing `DISCOURSE_TYPE_ICONS` map; multi-edge cases show a small count next to the icon (no count when N=1). Clicking the badge opens a sidebar `.discourse-edges-card` (sibling chrome to `.xc-edge-card` from clickable on-map edges) with the discourse pill → FC pill header followed by one section per edge: edge-type label, optional `nature` pill (bears-on edges), description prose, optional `discharge_status` block (polymorphic; rendered via the existing `renderDischargeStatus` helper), optional `cell_refs` as pills grouped under the FC, and optional citations. Hash routing `#/discourse-edges/<node-id>/<fc-id>` deep-links to the pair-card; the parent discourse-node selection is retained so the ring stays lit while the card is open. Touched six files: `explorer-data.js` (new `discourse_edges_by_fc_pair` index in `augmentDataset`, `state.selectedDiscourseEdgesPair`, parse/write for the new hash with precedence `selectedFC > selectedEdgeId > selectedDiscourseEdgesPair > selectedDiscourseNode > glossary`), `explorer-discourse.js` (new `renderDiscourseEdgeBadge(fc)` returning the HTML or `''`, `selectDiscourseEdges` / `clearDiscourseEdgeSelection` lifecycle pair, `renderSidebarDiscourseEdges(nodeId, fcId)` card renderer, and a `DISCOURSE_TYPE_LABELS_EDGE` map for the edge-type pretty labels; `clearDiscourseSelection` extended to also clear the dependent pair), `explorer-map.js` (renderTile injects the badge HTML inside the tile element above `.tile-corner`; post-render click delegation on `.dc-edge-badge` with `event.stopPropagation()` so the whole-tile click still fires elsewhere), `explorer-sidebar.js` (new `'discourse-edges'` case in `renderPanel`, deep-link restore branch in `renderAll` between the edge and discourse-node branches, symmetric `selectedDiscourseEdgesPair = null` clears in `selectFC`, `selectCell`, `selectCrossClassEdge`, `clearSelection`, and `switchSidebarPanel`), `update-c-edges.css` (`.dc-edge-badge` chrome with hover scale + glow, `.discourse-edges-card` block mirroring `.xc-edge-card` patterns, per-edge `.de-card-edge` sub-sections, the `.pt-tile.discourse-highlight .fals-flag` 28-px margin nudge), and `Map_v34_explorer.html` (opportunistic fix of the stale "8 phenomenon-to-phenomenon edges" → "7" in both spots in the help overlay). Real-data check across v34 surfaced a useful structural fact: **all 31 (node, FC) pairs are single-edge in v34** — the multi-edge UI is correct but currently unexercised. Candidate-foundational programs (AdS/CFT, NCG, tensor networks, causal sets, SU(5) GUT, qm-foundational) have zero FC-direction discourse-edges and produce a clean no-badge empty state when selected — matches the known authoring gap in `PROJECT_GOAL_SUPPLEMENT.md` §2.1. QFT lights up 6 FCs; QG-frontier 4; topological-phases-classification + stat-mech 3 each.

### What's queued (live)

- **Update C — done.** All five Update C items shipped: glossary panel, clickable on-map edges, typography pass, in-tile cell viz, discourse-highlight ring badges. With the ring badges shipped, the explorer surfaces every layer of the dataset that's discoverable through the existing data shapes — no remaining "data present but UI invisible" gaps. The "What was built in Update C — discourse-highlight ring badges" section later in this doc records what shipped in the final pass.
- **Step 5 — open the review pathway** (live, the next milestone — text-and-process work, not code). Update B made the discourse layer navigable, the Update C glossary lowers the explanation overhead for unfamiliar physicist reviewers encountering map-internal vocabulary like "discharge status" or "candidate-foundational," Update C clickable edges surfaces previously-buried citation chains that subfield specialists will want to verify against their literature, the typography pass makes math-heavy prose actually readable, the in-tile cell viz turns the map's empirical state into a glanceable summary, and the ring badges give a one-click path from any selected discourse node to the specific edges connecting it to each lit-up FC. Three deliverables: GitHub Issues templates for cell errata / new-FC proposals / cross-class edge proposals; `/methodology/EDITORIAL.md` describing the editorial process; outreach copy + the live explorer link sent to 2–3 working physicists in different subfields. The "Step 5" sketch later in this doc has the operational details. A fresh chat handles this cleanly — no need to maintain explorer-code context.
- **v34 data-side double-escape normalization PR** (text-and-data work, optional). The typography fix's `\\` → `\` collapse in `formatPara` is the presentation-layer workaround for the dataset's inconsistent escape levels in math fields; a source-data PR would let the workaround eventually be removed. Firewall-gated; needs proper PR review through CI. Lower priority than Step 5.
- **Outstanding from this pass:** the worker's `server_info` prose at `https://map-of-physics.eddie-8e5.workers.dev` still says "8 phenomenon-to-phenomenon edges" — the live data has 7, and the explorer's help-overlay was fixed in the ring-badges commit, but the worker text needs the same one-line fix via the Cloudflare dashboard. Verified by enumerating cross-class edges via the MCP server. Trivial one-line edit; do it next time the worker is touched.
...
3. Tell it which section of this doc the work resumes at — e.g., *"Pick up at Step 5 as described in EXPLORER_HANDOFF.md, starting with EDITORIAL.md and the GitHub Issues templates."*
...
**update-c-edges.css** (~7.4 KB; was 3.5 KB before ring badges doubled it) — clickable-on-map-edges styling: `.pt-overlay-svg path.phen-phen-hit` and `.phen-phen-hit.arrow-hit` for wide transparent SVG hit-areas, `g.phen-phen-edge-group:hover path.phen-phen-edge` for the hover thickening, `.dc-edge.dc-edge-clickable` block-level hover/focus chrome inside FC cards, `.detail-card.xc-edge-card` with `.xc-edge-endpoints`, `.xc-edge-arrow`, `.xc-edge-meta`, `.xc-targeted-by`, `.xc-cells-grp`, `.xc-citation-list` for the cross-class edge card. **Update C ring badges added:** `.dc-edge-badge` chrome (absolute top-right of tile, 1.5px solid amber border, faint amber fill `rgba(184,101,26,0.18)`, z-index 4 to sit above the discourse-highlight `::after` dashed ring, hover scale + glow); `.dc-edge-badge-icon` and `.dc-edge-badge-count` sub-element rules; `.pt-tile.discourse-highlight .fals-flag { margin-right: 28px }` so the falsified flag shifts left when both are present (badge ~22-26px wide depending on count, 28px clearance comfortably covers the wider case); `.detail-card.discourse-edges-card` sidebar card chrome (sibling to `.xc-edge-card`, distinguished by `border-left: 3px solid var(--discourse-ring)`); per-edge `.de-card-edge` sub-section block with type label, head row, description, discharge block, cells group, citations list — all mirroring the `.xc-*` patterns from cross-class but with `.de-*` prefixes since this is one card with multiple edge sections, not one card per edge.
...
| Add inline glossary cross-linking inside cell descriptions / discourse cards (deferred — see Update C glossary's "What was built" note) | `explorer-data.js` (`formatPara` extension + alias scan index in `augmentDataset`) + `explorer-glossary.js` (click target resolution) |
...
| Adjust ring-badge position (corner inset, size) | `update-c-edges.css` (`.dc-edge-badge` `top` / `right` / `padding`) |
| Change ring-badge color or hover behavior | `update-c-edges.css` (`.dc-edge-badge` + `:hover` + `:focus-visible`) |
| Adjust the falsified-flag margin nudge when both badge + flag are present | `update-c-edges.css` (`.pt-tile.discourse-highlight .fals-flag` `margin-right`) |
| Change which icon the badge shows (per node-type) | `explorer-discourse.js` (`DISCOURSE_TYPE_ICONS` — shared with discourse-card headers, so changes affect both surfaces) |
| Adjust ring-badge count rendering (always-show vs N>1 only) | `explorer-discourse.js` (`renderDiscourseEdgeBadge`) |
| Edit a discourse-edge type's pretty label in the badge card | `explorer-discourse.js` (`DISCOURSE_TYPE_LABELS_EDGE`) |
| Tweak discourse-edges card layout (header pills, per-edge sections, citations style) | `explorer-discourse.js` (`renderSidebarDiscourseEdges`) |
| Restyle discourse-edges card chrome (`.discourse-edges-card`, `.de-card-*`) | `update-c-edges.css` |
| Change discourse-edges hash format | `explorer-data.js` (`parseHash` / `writeHash` — the `'discourse-edges'` branch) |
| Change which edges the badge counts (e.g. exclude certain edge types) | `explorer-data.js` (`augmentDataset` — the `discourse_edges_by_fc_pair` loop) |
...
**What's deferred from this pass — discourse-highlight ring badges.** A small clickable corner-of-tile element on FCs lit up by a discourse-node selection, opening a card that explains why this FC connects to the selected discourse node. The xc-edge-card chrome built here (header pills + structured sections + cell-ref pills + citations) is directly reusable; only the trigger and card content differ. **Shipped in the subsequent ring-badges pass — see "What was built in Update C — discourse-highlight ring badges" below.**
...
## What was built in Update C — discourse-highlight ring badges

Concise reference for what shipped in the final Update C item.

**Goal shipped.** When a discourse node is selected, the amber discourse-highlight ring already lit up every connected FC. The ring was visually clear about *which* FCs connected but said nothing about *how* — to see the edges, the user had to navigate back to the discourse card and scan its cell-ref pills. The ring badge closes the loop: a small clickable amber chip in the top-right corner of each lit-up tile opens a sidebar card listing every edge between the selected discourse node and that FC, with full description prose, optional `nature` + `discharge_status` for bears-on edges, optional cell-ref pills, and optional citations.

**Badge placement and chrome.** Absolute-positioned top-right of the tile (`top: 3px; right: 3px;`), z-index 4 so it sits above the discourse-highlight `::after` dashed inner ring. 1.5px solid amber border + faint amber fill (`rgba(184,101,26,0.18)`) using the same `--discourse-ring` color as the ring itself. Hover scale (1.08) + glow. Icon mirrors the *selected discourse node's type* using the existing `DISCOURSE_TYPE_ICONS` map (⌬ architecture, ✕ frontier, ◇ totality, ▣ regime, ⚙ program) so the badge visually says "this FC connects to the architecture / frontier / etc. you selected." Count next to the icon when N>1; no count when N=1 (the chip itself is the indicator).

**Falsified-flag overlap handled by margin nudge.** Some FCs have falsified predictions (modified-gravity-alternatives, proton-decay-searches, dark-matter-candidates, etc.) — their `.fals-flag` lives in `.tile-corner`'s right slot. The ring badge would visually overlap. Fix: `.pt-tile.discourse-highlight .fals-flag { margin-right: 28px }` shifts the flag left whenever the discourse-highlight class is on the tile (which is always when the badge is rendered). 28px comfortably covers the badge width across the N=1 and N>1 cases.

**Multi-edge case.** Single badge per (discourse-node, FC) pair, opening a card with one section per edge. The badge count text reflects the edge count. The card renders each edge as its own `.de-card-edge` block — edge-type label, optional nature pill (bears-on only), description prose, optional discharge-status block (polymorphic shape handled by the existing `renderDischargeStatus` helper), optional cell-ref pills grouped under the FC, optional citations. Verified across v34: **zero multi-edge pairs in the live dataset** — every (node, FC) pair carries exactly one edge. The multi-edge UI is correct but unexercised; future data shapes may exercise it.

**Hash routing.** `#/discourse-edges/<node-id>/<fc-id>` deep-links to the pair card. Pair-keyed, not edge-id-keyed, since the badge addresses the (node, FC) relationship rather than a specific edge — the card may render 1+ edges for the same pair. `parseHash` sets both `selectedDiscourseEdgesPair` AND `selectedDiscourseNode` so the parent ring stays lit when restored. `writeHash` precedence: `selectedFC > selectedEdgeId > selectedDiscourseEdgesPair > selectedDiscourseNode > glossary` (the pair card is strictly more specific than the bare node selection).

**Decisions locked in by this build:**

- **Badge position: absolute top-right corner, not in-row.** Considered placing in the `.tile-corner` flex row between cell-ct and the fals-flag, but the corner row's 9px font-size would have made the badge too small to be a comfortable click target. Absolute positioning gives a 16-18px square chip — proper button size — at the cost of needing the fals-flag margin nudge. Right tradeoff for usability.
- **One badge per FC, opening a multi-edge card.** Alternative was one badge per edge — would have given more granular hash URLs but more visual noise on multi-edge FCs. Since v34 has zero multi-edge pairs, the per-edge approach would have visually behaved identically; but the per-FC design extends cleanly to future multi-edge data without re-architecting the click surface.
- **Card renderer lives in `explorer-discourse.js`, not `explorer-sidebar.js`.** sidebar.js was at 67 KB before this pass; the card renderer (~80 lines) plus state lifecycle (~30 lines) would have pushed it within 5 KB of the 80 KB ceiling. discourse.js has plenty of headroom (37 KB → 46 KB after this pass), and all the pill / discharge / cell-ref helpers the card uses already live there.
- **Selection state lives in `explorer-sidebar.js` (the `select*` / `clear*` functions).** Even though the renderer is in discourse.js, the lifecycle pair joins the other selectors in sidebar.js to keep the symmetric-clear pattern centralized. discourse.js's `selectDiscourseEdges` is technically defined there; sidebar.js's `selectFC` / `selectCell` / `clearSelection` / `switchSidebarPanel` all add a single-line `state.selectedDiscourseEdgesPair = null` to maintain the clear-on-competing-selection invariant.
- **Badge is a `<button>`, not a `<div>`.** Native keyboard activation (Enter / Space), native focus ring (overridden with `:focus-visible` outline), native semantics for screen readers. Click handler attached with `stopPropagation` so the underlying tile's `selectFC` doesn't also fire.
- **No tooltips on badges — `title` attribute carries the count + node label** ("Show 2 discourse-edges from QFT"). Tooltip would compete with the tile-hover tooltip; the card opening on click is the discoverable affordance.
- **Help-overlay phen↔phen edge count fixed in same commit.** The "8 phenomenon-to-phenomenon edges" → "7" follow-up in `Map_v34_explorer.html` is a free-rider opportunistic fix (handoff-recorded outstanding follow-up). Same commit because the badge work touches no other HTML.

**Data layer additions in `augmentDataset`.**
- `discourse_edges_by_fc_pair[<node>|<fc>]` map: keys are `"<node-id>|<fc-id>"`, values are arrays of edge records with `direction` + `neighbor` (same shape as `discourse_edges_by_node` entries). 31 keyed pairs in v34, all single-edge. Used by the badge renderer (to determine presence/absence) and the card renderer (to list edges for a (node, fc) pair).

**State + hash routing additions.**
- `state.selectedDiscourseEdgesPair: { nodeId, fcId } | null` added to the singleton.
- `state.activePanel` extended with `'discourse-edges'`.
- `parseHash` branch: `parts[0] === 'discourse-edges'` sets both `selectedDiscourseEdgesPair` and `selectedDiscourseNode` (so the ring stays lit on restore), `state.activePanel = 'discourse-edges'`.
- `writeHash` precedence: `selectedFC > selectedEdgeId > selectedDiscourseEdgesPair > selectedDiscourseNode > glossary`. Hash format `#/discourse-edges/<node-id>/<fc-id>`.
- Symmetric clears wired across `selectFC`, `selectCell`, `selectCrossClassEdge`, `clearSelection`, `clearDiscourseSelection`, `switchSidebarPanel`. Each sets `selectedDiscourseEdgesPair = null` directly (or calls `clearDiscourseEdgeSelection`).
- `renderAll` boot branch: between the edge and discourse-node restore branches; if hash had `#/discourse-edges/<n>/<fc>`, lookup the node + FC and render the card.

**Smoke test (dev-only, not in repo).** 52-assertion JSDOM run against a synthetic stub dataset of 4 FCs + 3 discourse nodes + 5 discourse-edges (including a multi-edge case from `qft` to `sm-rep-content` and a `bears-on` edge with `discharge_status` to exercise the polymorphic field-handling). 14 sections: data-layer index population (7), state singleton init (1), badge presence/absence by selection state (8), badge data attributes (2), click delegation + stopPropagation (3), card content rendering (8), bears-on edge nature + discharge (2), hash write (4), hash parse (3), deep-link restore via renderAll (2), symmetric clears across 6 paths (6), help-overlay fix (2), re-render idempotence (1), node module syntax checks. **Plus an end-to-end real-data check** (8 assertions) against the actual v34 dataset that surfaced the structural finding: all 31 pair entries are single-edge, candidate-foundational programs produce a clean empty-state with no badges, QFT lights up 6 FCs, QG-frontier 4, topological-phases-classification + stat-mech 3 each. **The 52-assertion synthetic-stub floor becomes the new baseline for future passes** that touch the ring-badge code, with the caveat that prior chats' smoke-test workspaces are not retained — so the cumulative-floor convention (Update B's 80, in-tile cell viz's 169) resets at each chat boundary. Future passes either extend from 52 (badge-scoped) or reconstruct their own coverage of unrelated surfaces from the patterns documented in "Smoke testing under JSDOM" below.

**Real-data findings worth flagging from the e2e check across v34.**
- **31 (node, FC) pair entries total, all single-edge.** Distribution: `{"1": 31}`. Multi-edge UI correct but unexercised in current data.
- **Candidate-foundational programs produce zero badges.** AdS/CFT, NCG, tensor networks, causal sets, SU(5) GUT, qm-foundational, jaynesian, rg-info, stat-mech (partial) all have either zero or very few FC-direction discourse-edges. SU(5) GUT specifically has its FC connections via cross-classification edges (the existing clickable-edges system surfaces those), not via uses/produces/bears-on discourse-edges. Selecting these programs lights up no ring at all — by design. Matches the known authoring gap in `PROJECT_GOAL_SUPPLEMENT.md` §2.1 ("authoring `produces-classification` and `bears-on` edges to the existing v13 candidate-foundational programs is the main gap in test 2.1").
- **Highest-connectivity discourse nodes by FC count:** QFT (6 FCs), QG-frontier (4 FCs), topological-phases-classification (3 FCs), stat-mech (3 FCs). The QFT badges show on tenfold-way, sm-rep-content, ade-lie-algebras, spacetime-symmetry-groups, freed-hopkins-cobordism, cft-bootstrap-exclusion-regions when QFT is selected.

---

## Step 5 — review pathway (live, the next milestone)

All Update C items have shipped. The explorer surfaces every layer of the dataset that's discoverable through the current data shapes — discourse cards, glossary, clickable cross-class edges, math-rendered prose, in-tile cell visualizations, and now the ring-badge cards for FC↔discourse-node connections. Step 5 opens the review pathway to working physicists. Three deliverables, in priority order:
...
Each module targets ≤ 60 KB. HTML skeleton ≤ 65 KB (currently **~52 KB** after the typography pass extracted three CSS chunks from the inline block — comfortably under the cap). The four CSS files are each well under the same target (`update-b.css` 16 KB, `update-c.css` 2 KB, `update-c-edges.css` 7.4 KB (doubled in the ring-badges pass from 3.5 KB), `update-c-typography.css` 7.5 KB). After ring badges, the six JS modules: `explorer-data.js` 32 KB, `explorer-discourse.js` 46 KB, `explorer-glossary.js` 12 KB, `explorer-map.js` 26 KB, `explorer-phenomena.js` 15 KB, `explorer-sidebar.js` 69 KB — sidebar.js is the closest to its ceiling (69 / 80 KB). If a single file approaches its cap, split it further. The monolith broke down at ~150 KB; modules feel comfortable up to ~60 KB. CSS files probably comfortable up to ~30 KB; beyond that, split by concern.
...
- Update B's smoke test had 80 assertions; Update C glossary added 46 against a narrower stub; Update C clickable on-map edges landed at 91 against a comprehensive stub; Update C typography pass landed at 123 then 129 after the escape-normalization patch; Update C in-tile cell viz landed at 169; Update C ring badges landed at 52 (focused stub, prior workspaces gone). **The cumulative-floor convention assumed prior workspaces persisted between chats; they don't.** Going forward: each pass writes a focused smoke test for its own additions (~30-50 assertions on the new code paths) plus light regression checks against the major prior surfaces (~5-10 assertions). See the "Smoke-test floor reset between chats" lesson below.
...
- **The smoke-test floor convention.** Update B landed at 80, Update C glossary added 46 against a glossary-specific stub (narrower scope), Update C clickable edges landed at 91 against a comprehensive stub, Update C typography pass landed at 123 then 129 after the escape-normalization patch, Update C in-tile cell viz landed at 169. Update C ring badges landed at 52 (focused stub; prior chats' workspaces gone — the cumulative count couldn't be re-built). **Going forward the convention is: each pass writes a focused smoke test for its own additions (~30-50 assertions) plus a light regression check against major prior surfaces (~5-10 assertions).** Document the assertion count, focus, and scope at the bottom of the relevant "What was built" section. The cumulative-floor convention assumed persistent per-pass workspaces; in practice the workspaces don't persist, so the convention should be per-pass-focus instead. Don't treat 169 (or any other number) as a strict floor for unrelated work.
...
- **Outstanding follow-up: worker `server_info` "8 phenomenon-to-phenomenon edges" preamble drift.** The HTML side of this was fixed in the ring-badges commit (both spots in the help overlay now say "7"), but the worker at `https://map-of-physics.eddie-8e5.workers.dev` still serves the stale "8" in its `server_info` text. Trivial one-line edit via the Cloudflare dashboard, but requires the maintainer's web-UI step rather than a git commit. Verified by enumerating all 97 cross-class edges via the MCP server: live count is 7. Each pass that touches the worker should fix this at the same time.

- **v34 has zero multi-edge (discourse-node, FC) pairs.** The ring-badge pass speculatively designed for a multi-edge case (one badge per FC opening a card that lists all edges) because I expected SU(5) GUT or similar candidate-foundational programs to have both `candidate-hosting` AND `produces-classification` edges to the same FC. They don't. All 31 (node, FC) pairs in v34 are single-edge. The multi-edge UI is correct but currently unexercised — the badge count never shows above 1, the card never renders 2+ `.de-card-edge` sections. Future data may exercise it; the design is forward-compatible. Lesson for future ring-badge-adjacent work: don't assume real data exercises the full code path; query the live dataset first.

- **Candidate-foundational programs have zero FC-direction discourse-edges.** AdS/CFT, NCG, tensor networks, causal sets, SU(5) GUT, qm-foundational, jaynesian, rg-info all produce zero ring badges when selected. SU(5) GUT specifically has its FC connections via cross-class edges (which the existing clickable-edges system surfaces), not via uses/produces/bears-on discourse-edges. This empty-state is **by design** — matches the known authoring gap in `PROJECT_GOAL_SUPPLEMENT.md` §2.1 ("authoring `produces-classification` and `bears-on` edges to the existing v13 candidate-foundational programs is the main gap in test 2.1"). The ring-badge UI doesn't need to do anything about this — when the user selects one of these programs, no ring lights up and no badges appear. The UI honestly reflects the data's state. Lesson: empty-states in the UI are often features, not bugs, when they reflect honest absences in the source data.

- **Badge positioning: absolute + margin-nudge composes cleanly with existing tile chrome.** The ring badge is absolute-positioned at `top: 3px; right: 3px;` with z-index 4 (above the discourse-highlight `::after` dashed ring at the same stacking context). When a falsified-flag is also present, a `.pt-tile.discourse-highlight .fals-flag { margin-right: 28px }` rule shifts the flag left within `.tile-corner`'s flex row. Three-element interaction (cell-ct on left, badge floating above-right, fals-flag in the flex row, shifted): zero CSS conflict because each element uses different positioning models (flex-row for cell-ct + fals-flag, absolute for badge, pseudo-element for the ring's dashed inner stroke). Pattern reusable for any future "conditional notification chip on a tile" requirement.

- **Card renderer in `explorer-discourse.js`, lifecycle in `explorer-sidebar.js`.** A new split-by-concern that scales as files approach their soft caps. The card renderer (~80 lines) depends on discourse-module helpers (`renderDiscoursePill`, `renderFCPill`, `renderDischargeStatus`, `renderCellRefPills`, `formatPara`); the lifecycle pair (`selectDiscourseEdges` / `clearDiscourseEdgeSelection`, ~30 lines) sits with the other selectors so the symmetric-clear pattern stays centralized. discourse.js grew 37 → 46 KB (headroom remaining), sidebar.js grew 67 → 69 KB (still safely under 80 KB ceiling). Lesson: when a new feature touches both helpers and state, locate the helpers with their natural neighbors and the state with the lifecycle conventions — the cross-module call graph is fine as long as each module exposes its functions as top-level `function` declarations.

- **`<button>` element for the badge — native semantics + idempotent click delegation.** Using `<button type="button" class="dc-edge-badge" data-disc-edge-fc data-disc-edge-node>` gives keyboard activation (Enter/Space → click), screen-reader semantics ("button"), and focus management out of the box. Click delegation happens in the post-render `forEach` loop on `.dc-edge-badge` selectors — re-running `renderMap` rebuilds the tile DOM, and the wiring loop attaches fresh handlers each time. No handler-stacking because each renderMap call produces fresh DOM. Pattern matches the existing `pt-tile` click wiring on the same parent.

- **stopPropagation on the inner button is essential.** Without `event.stopPropagation()` in the badge click handler, clicking the badge would ALSO fire the underlying tile's `selectFC` click handler — opening the FC sidebar instead of the discourse-edges card. Same pattern as `.edge-target` inside `.dc-edge` cards, and the cross-class edge badge inside FC cards. The smoke test explicitly verifies both directions: (1) badge click fires `selectDiscourseEdges` and does NOT fire `selectFC`, (2) tile click on a non-badge area still fires `selectFC` normally. If a future pass adds yet another nested clickable element, replicate the pattern.

- **Smoke-test floor reset between chats.** The handoff's prior convention ("Update B's 80, in-tile cell viz's 169 — extend in next pass") assumes prior smoke-test workspaces persist into the next chat. They don't — `/home/claude/` is per-chat. Ring-badges' synthetic-stub smoke test landed at 52 assertions, scoped to the new feature. **For future passes the convention should be: each pass writes a focused smoke test for its own additions (~30-50 assertions covering the new code paths) PLUS a small regression check against the major prior surfaces (~5-10 assertions) to confirm nothing critical broke.** That gives most of the value of a cumulative floor without depending on workspace persistence. Document the focus and scope at the bottom of the relevant "What was built in Update C — …" section, as ring-badges does.

- **The handoff sketch turned out mostly right but had to be adapted at one point.** The pre-coding proposal in the chat suggested putting the badge "inside `.tile-corner`'s flex row between cell-ct and fals-flag" as Option A; I picked Option B (absolute top-right) after the user's "1. top right" choice and confirmed with the corner row's 9px font-size that it would have been an uncomfortably small click target. Lesson: when proposing a placement, gather the layout constraints (font-sizes, line-heights, the existing absolute-positioned siblings) before locking the placement. The "top right" user instruction admitted two implementations; the right one for usability needed extra reasoning about click-target size that the original sketch didn't include.

- **`DISCOURSE_TYPE_LABELS_EDGE` is a parallel map to `DISCOURSE_TYPE_LABELS` (which is for node types).** Node types: architecture / open-frontier / totality / regime-content / candidate-foundational / experimental-program. Edge types: uses-classification / produces-classification / bears-on / emergence / candidate-hosting / candidate-targeting / etc. The two maps don't collide and don't share entries; they coexist as module-local constants in `explorer-discourse.js`. Lesson when extending: don't try to merge them — they serve different render surfaces.

- **The handoff's "free-rider fix" pattern.** When a pass touches an HTML file for unrelated reasons, look at the file's outstanding-follow-ups list and roll in any cheap fixes (the "8 → 7" edit was a single-character swap repeated in two spots, costs nothing extra to include). Ring badges didn't strictly need an HTML edit (badge is JS-injected, CSS is in update-c-edges.css), but the outstanding follow-up was sitting there and the maintainer had already flagged it. **Reverse pattern when a pass doesn't touch the relevant file** (e.g. ring badges didn't need to touch the worker.js, so the worker's `server_info` "8" stays unfixed for now): leave it on the outstanding list for the next pass that does touch the file.
