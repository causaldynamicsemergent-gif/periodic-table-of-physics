...
**Currently deployed on the live site: Update A.1.** Any chat that ships a new build is responsible for updating this line in the same commit.
...
The dataset already contains the discourse-layer nodes the v4 explorer doesn't render. Verified counts against the live MCP server (May 2026): 11 architectures (string theory, asymptotic safety, loop quantum gravity, etc.), 11 open frontiers (quantum gravity, hierarchy problem, baryogenesis-mechanism, etc.), 7 experimental programs (LHC, LIGO, Planck, etc.), 6 totality-approaches (bh-thermodynamics, cosmological-models, turbulence, koide-formula, muon-g-2, chpt), and 6 **regime-content** nodes (sm-electroweak, qcd, higgs, classical-gravity, hilbert-measurement, equilibrium). Regime-content is easy to miss — the type name doesn't appear in older project descriptions, but the 6 nodes are real and carry equations, loose-ends, and incident edges that participate in Update B's rendering.

71 nodes total: 30 FC + 11 architectures + 11 frontiers + 7 programs + 6 totalities + 6 regime-content.

**Edge taxonomy.** The discourse layer has at least 11 edge types, of which only some were in earlier drafts of this doc. Confirmed types include `cross-classification` (already rendered in v4, 97 edges, 4 statuses), `bears-on` (FC → frontier/totality), `uses-classification` (architecture → FC, may carry `cell_refs`), `produces-classification` (experimental-program → FC), `candidate-targeting` (architecture → frontier, carries graded `discharge_status` across local/global/universal), `candidate-hosting` (architecture → architecture — note: the dataset uses `candidate-hosting`, not "hosts"), `open-frontier-architecture-edge` (frontier ↔ architecture, often labeled "fails-to-span"), `open-frontier-content-edge` (frontier ↔ regime-content), `emergence` (architecture → regime-content), `cross-architecture-emergence` (regime-content → totality-approach), `multi-architecture-interference-edge` (totality → architecture). **Before planning the rendering, a B-implementation chat should query the MCP server for the complete current edge taxonomy.** Total non-cross-class discourse edges: ~89. Combined with the 97 cross-class edges already rendered, that's the 186 edges in the dataset.

PROJECT_GOAL §8's "Working" five-click test requires a user to get from any frontier to (a) its structural reason, (b) the classifications bearing on it, (c) the targeting programs in fewer than five clicks per leg. That defines the lower bar for Update B. The §8 "Distinctive" condition — comparing two structurally-distant frontiers classified under the same structural-reason kind — implies an additional UI affordance: a filter or group-by on `structural_reason_category`, slotted into the toolbar's rows-by chips or into Browse. Not just per-node display; a real filter.

**Design question — how do the two layers sit on screen?** Three options worth weighing:
...
- **Option c** — discourse nodes live in Browse and in the sidebar, not as a parallel grid. The FC map remains the only persistent visual anchor. Selecting a discourse node highlights connected FCs on the existing map.

A planning pass against the verified data (May 2026) recommended **a refined option (c)**: the FC map stays exactly as A.1 ships it; discourse nodes get five new Browse sub-tabs (Architectures, Frontiers, Programs, Totalities, Regime-content); selecting a discourse node opens a sidebar detail card and highlights connected FC tiles on the map via the multi-ring `box-shadow` mechanism A.1 already uses for phenomena. Reasons: drawn-edge approaches don't scale to 89+ edges across 10+ types (option a), unified grids risk conflating the openness-axis / content-axis distinction PROJECT_GOAL §4 makes load-bearing (option b), and (c) preserves A.1's fit-to-view behavior which assumes full-pane map width.

The recommendation is planning input — a B-implementation chat should re-derive the choice from fresh evidence and the maintainer's preferences. The mockup-three-options-and-pick path remains valid. Whatever's chosen, the rendering should visually distinguish the discourse-selection overlay from the phenomena overlay (different ring colors or styles), since both use the same map tiles as their canvas.

An older mockup of the project (the v12 review deck) used Option (a) with edges drawn in SVG between two panes; if the maintainer has it locally, they can attach it to the refactor chat for reference, but the refactor doesn't require it.
...
**Cosmetic and presentation-layer changes are not gated by the firewall.** Adjusting phenomenon-category colors, tile styling, the About / Education / Research panel copy, the legend layout, or any other purely visual concern ships through a normal explorer commit — no MCP query, no schema PR, no CI consideration beyond the explorer's smoke test. The firewall only applies to changes that touch cell content, axis labels, citations, edge semantics, or the consolidated JSON itself.
...
**Until the refactor lands, the four `explorer-*.js` URLs above will 404.** Treat the monolith HTML as the only fetch target. The deployment status line at the top of this doc tells you whether the refactor has shipped.
