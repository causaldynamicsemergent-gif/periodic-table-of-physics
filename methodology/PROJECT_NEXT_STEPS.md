...
**Sub-PR E7 — find_discriminating_experiments view in the explorer.** *Default maintainer suggestion; the remaining discretionary explorer follow-ups (E6 / E8), the recipes-and-sessions-to-About-panel extraction, the cross-FC pattern layer trigger watch, and the Cloudflare-Worker-proxied chat are the equally-eligible alternatives if the maintainer prefers a different priority.*

Prompt template at §5.14. Description-rewrite content housekeeping closed 2026-05-27 — the modified canonical data file (`data/Map_v34_consolidated.json` at v96) was shipped complete via present_files carrying 89 in-place edits across 38 description fields on 36 resolves-type edges and 2 dark-matter-carrier if_real_implies implications, eliminating 101 schema-name leak occurrences against the four target patterns in `PHYSICIST_FACING_VOCABULARY.md` §3 (`exclusion_only`, `predictions_per_program`, `bound_direction`, bare `resolves edge(s)` plural-noun usage). The companion CHANGELOG at `/methodology/DESCRIPTION_REWRITE_CHANGELOG_2026-05-27.md` records the 89 (before, after) pairs for audit and option B follow-up reference. The cleaner descriptions render in the explorer's resolves-edge rows and implication trees per sub-PRs E2 / E3 (DISCHARGED 2026-05-27); E7 inherits the cleaner surface as the substrate for its panel.

A stowaway sub-PR landed 2026-05-28 between this session and the previous: the §1.4 location-drift was corrected (the example sessions live at `/methodology/`, not at the imagined-but-never-instantiated `/methodology/SESSIONS/` that earlier amendments had recorded), and a second curated example session was authored at `/methodology/2026-05-26-vocabulary-discipline-origin.md` — the meta-conversation that produced `PHYSICIST_FACING_VOCABULARY.md`, `TRACK_4_USE_SIDE_ARTIFACTS.md`, and the recursive closing-prompt pattern. The corpus now has two edited-register transcripts; the cc-frontier walk shows the disciplines working, this new one shows where they came from. The next default priority is still E7.

E7 surfaces the project's flagship Phase C AI-first query: pick two experimental programs, see what they jointly resolve with per-program SM-vs-BSM prediction differentials surfaced. The strongest concrete demonstration target for outreach (per `PREDICTIVE_LAYER_PHASE_C_HANDOFF.md` §3 and `EXPLORER_PHASE_BC_HANDOFF.md` §2.6). The substrate is ready: 38 resolves edges, 13 with per-program competing-prediction entries populated, multiple discriminating-experiments clusters across HK ↔ DUNE ↔ JUNO (mass-ordering and p → ν̄K⁺), gw-ground-network (NS-EOS, seed-formation), CMB-S4, Rubin parallel-resolves on cc-frontier, and the muon-g-2-continuation TI vs BMW SM-side adjudication. Expected diff shape: one new JS module `explorer-discriminating.js`, one new CSS file `update-e7.css`, plus a script/link tag pair in `Map_v34_explorer.html` and optionally an index update in `explorer-data.js`. Alternative post-description-rewrite priorities for maintainer selection: **E6** (rank_by_scale overview panel — useful for "what's the coverage by kind" at a glance); **E8** (banner refresh — one-file housekeeping, lowest cost); **recipes-and-sessions-to-About-panel extraction** (successor to E8); **cross-FC pattern layer trigger watch** per `CROSS_FC_PATTERN_LAYER_SCOPE_MEMO.md` §10.1; **Cloudflare-Worker-proxied chat** (successor to Track 4 §1.3, only if the maintainer decides the three-click setup friction binds outreach). The maintainer chooses sequencing among these.
...
1. **Sub-PR E7 (find_discriminating_experiments view)** — flagship Phase C AI-first query surfaced as an explorer panel. Pick two experimental programs, see what they jointly resolve with per-program SM-vs-BSM prediction differentials. Strongest concrete demonstration target for outreach now that the resolves-edge cards read cleanly. Template at §5.14.

Once E7 closes, the remaining post-description-rewrite priorities each currently in §4 as eligible-but-not-promoted become the eligible queue. Maintainer chooses sequencing; the natural shortlist:

- **Sub-PR E6 (rank_by_scale overview panel).** Discretionary net-new view beyond explorer Phase B+C surfacing closure. Triggers met; eligible-on-request. Useful for the working physicist who wants the "what's the coverage by kind?" question answered in one glance.
...
- **Description-rewrite content housekeeping option B follow-up** — adjacent leak patterns the discipline also catches but were out of the option A scope shipped 2026-05-27: PPP abbreviation, T-convention references (T30/T31/T33), §4 admissibility-test references, sub-PR landmark caps paragraphs in their broader form, the two remaining `field` data-structure noun uses. Recorded in `DESCRIPTION_REWRITE_CHANGELOG_2026-05-27.md` at the closing section. Cheap to fold into any future content-housekeeping pass.
- **Authoring follow-up: experimental-coverage edges for `cell-nu-theta23-atmospheric`** — surfaced as a descriptive observation under the firewall during the 2026-05-26 vocabulary-discipline-origin session, recorded in that transcript's Turn 2 and closing paragraph. The cell has zero experimental-coverage edges despite being measured by HK and DUNE in the same beam channels that resolve `cell-nu-deltaCP` and `cell-nu-mass-hierarchy`. Authoring the HK and DUNE experimental-coverage edges into this cell is eligible on physics-content grounds; the firewall (`META_v21_1_methodology_firewall.md` §2) forbids a recommendation, so the trigger is the maintainer's separate decision. Small sub-PR if pursued — 2 edges with the per-program competing-prediction lists empty per the same §4(2) admissibility pattern the existing δ_CP and mass-ordering edges follow.
...
**Sub-PR E6 (rank_by_scale overview panel) and E8 (explorer banner + About panel refresh).** Discretionary net-new views / housekeeping beyond explorer Phase B+C surfacing closure. Triggers met; eligible-on-request and surfaced in §3 as part of the post-E7 shortlist. Not currently promoted to priority 1 because E7 is the maintainer-confirmed default.
...
### 5.14 Sub-PR E7 (find_discriminating_experiments view in the explorer)
...
### 5.15 First cross-FC sub-PR (anomaly-content virtual-FC, when triggers fire)

[Template to be drafted by the session that verifies the §10.1 triggers fire. The template body is appended to this document when the trigger conditions are met. Bumped from §5.13 → §5.14 on 2026-05-27 when §5.12 (example-sessions DISCHARGED) and §5.13 (description-rewrite template body) were added during the Track 4 §1.4 closure; bumped from §5.14 → §5.15 on 2026-05-27 when §5.13 was DISCHARGED and §5.14 (E7 template body) was drafted during the description-rewrite housekeeping closure session.]
...
3. **Section §5 (prompt templates) grows as new actions become eligible.** When a session ships a sub-PR that promotes a pending action to eligible, that session also drafts the §5 template for the new action if it doesn't exist. Templates §5.14 (and later) are placeholders or recently-drafted bodies that get filled in or marked DISCHARGED when their actions become eligible / ship. Discharged templates (§5.1, §5.2, §5.3, §5.4, §5.5, §5.6, §5.7, §5.8, §5.9, §5.10, §5.11, §5.12, §5.13 as of 2026-05-27) are retained with a DISCHARGED header so future sessions can read them as historical reference for analogous operations.
