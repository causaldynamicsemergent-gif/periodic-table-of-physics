...
**Track 4 §1.4 (curated example sessions) — workstream 2.4.** *Default maintainer suggestion; post-Track-4 priorities are the equally-eligible alternative if the maintainer prefers to defer §1.4.*

Prompt template at §5.12. Track 4 §1.3 (guided chat entry point) closed 2026-05-27 — the entry-point page landed at `/explorer/ask-the-map.html` with three setup steps, the project-tuned system prompt baking in the vocabulary discipline and a MUST-include-explorer-deep-link requirement, six starter prompts mirroring the recipes' six query patterns, and an `ask the map →` discovery link added to the explorer header. The hosting decision was option (c) of the three candidates (static instructions page); option (b) (Cloudflare-Worker-proxied zero-friction chat) was queued in `TRACK_4_USE_SIDE_ARTIFACTS.md` §4 as a successor follow-up if the project decides the three-click setup friction is the binding outreach cost. With §1.1, §1.2, and §1.3 all shipped, §1.4 example sessions is the only remaining Track 4 use-side artifact. Once §1.4 closes, the post-Track-4 priorities become the eligible queue: E6 (rank_by_scale overview panel) and E7 (find_discriminating_experiments view) discretionary explorer follow-ups; E8 (explorer banner + About refresh) one-file housekeeping; cross-FC pattern layer trigger watch per `CROSS_FC_PATTERN_LAYER_SCOPE_MEMO.md` §10.1 (Phase C closure and explorer surfacing closure triggers met; schema v20 / four-FCs-stable / validator-clean still gate); the description-rewrite content sub-PR queued during E2/E3/E5 closure (schema-name leaks in resolves descriptions and partial overlap in implication descriptions); and the recipes-to-About-panel extraction sub-PR queued during §1.2 closure. The maintainer chooses sequencing among these after §1.4 lands.
...
*State:* Four artifacts named. Three shipped 2026-05-27: §1.1 essay at `/methodology/USE_THE_MAP_ESSAY.md` (89 lines, five worked examples covering the three-channel mass-ordering landscape, the two-channel strong-CP-problem resolver, the dual-frontier EDM program, the sterile-neutrino cross-classification anchor, and the leptogenesis conditional-consequences chain — drafted against canonical v95 with explicit deep links to the seventeen-file E5 explorer build); §1.2 query recipes at `/methodology/QUERY_RECIPES.md` (93 lines, six paragraph-length cross-cutting query patterns each with a worked v95-verified example, covering the experimental-coverage lookup, cross-classification anchor lookup, conditional-consequences lookup, discriminating-experiments lookup, scale-ranking lookup, and falsified-predictions catalog); §1.3 guided chat entry point at `/explorer/ask-the-map.html` (416 lines, self-contained with inline CSS in the explorer's visual language and inline copy-to-clipboard JavaScript, three setup steps with copy buttons for the catalogue server URL and the project-tuned system prompt, six starter prompts mirroring the recipes' six query patterns, a verify-the-claims sidebar pointing at the explorer / repo / issues tracker; hosting decision: option (c) static instructions page; discovery surface: `ask the map →` link added to the explorer header). One remains pending: §1.4 curated example sessions (transcripts of physicist-style chats edited for publication; the live chat entry point now exists as the recording surface for these).

*Eligible action:* Track 4 §1.4 (example sessions, default). The previous-session alternative `Track 4 §1.3` has shipped; the new eligible alternative is to defer §1.4 in favor of post-Track-4 priorities (E6/E7/E8 discretionary explorer follow-ups; cross-FC pattern layer trigger watch; content-rewrite housekeeping; recipes-to-About-panel extraction) — but §1.4 is the natural close of the Track 4 use-side queue and the default maintainer suggestion.
...
1. **Track 4 §1.4 (curated example sessions)** — workstream 2.4. Highest priority by maintainer default: the only remaining Track 4 use-side artifact. Edits 4–6 worked test sessions into publishable form, demonstrating the dataset's moves and the vocabulary discipline in practice. Now that the live chat entry point exists, candidate transcripts can be captured directly through it rather than synthesized — the entry point produces transcripts, the transcripts demonstrate the entry point. Template at §5.12.

Once §1.4 closes, the post-Track-4 priorities (each of which currently sits in §4 as eligible-but-not-promoted) become the eligible queue. Maintainer chooses sequencing; the natural shortlist:
...
- **Description-rewrite content housekeeping** — schema-name leaks in 17–28 of the 38 resolves descriptions plus a small number of implication descriptions, queued during E2 / E3 / E5 closure. Blocks nothing; can ship anytime; mechanical pass through the data file.
- **Recipes-to-About-panel extraction** — successor to E8 (banner refresh). Condensed subset of `/methodology/QUERY_RECIPES.md` extracted into the explorer's About panel. Out of Track 4 scope; surfaced as a queued follow-up during §1.2 closure.
...
- **Cloudflare-Worker-proxied chat (option (b) follow-up to §1.3)** — only if the project decides the three-click setup friction in the shipped option (c) is the binding outreach cost. Multi-deliverable sub-PR sized like its own workstream (worker + chat UI + auth/rate-limit + ongoing API cost). Worth doing only after the example sessions §1.4 reveal whether the friction matters.
