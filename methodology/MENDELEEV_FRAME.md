...
**Mode-3 (multi-face configuration) is the most Mendeleev-faithful version of M1**, because it matches what Mendeleev actually did: construct a configuration and let the gap emerge. The surface that performs it is the **configuration builder** — the substrate's central Mendeleev surface. A worked illustration: a configuration cutting `dark-matter-candidates`, `compact-astrophysical-objects`, and `sm-rep-content` along a gravitational-coupling axis. The substrate already holds the literature-anchored relations these three classifications sit in — compact objects derive their matter content from SM rep content and from hadronic states, and specialize the diffeomorphism structure of spacetime (`edge-xc-compact-astro-derives-sm-partial`, `edge-xc-compact-astro-derives-hadrons-partial`, `edge-xc-compact-astro-specializes-spacetime`). When the configuration is laid out, a position with structural reason but no occupant is a substrate output — a `conjectured-by-pattern` cell generated at query time, with its properties derivable from the literature-anchored content of the surrounding cells.

This is why **the `conjectured-by-pattern` field standing at zero cells today is correct, not a deficiency.** The category populates at configuration time, when the configuration builder surfaces a gap — not at content-authoring time. Pre-authoring `conjectured-by-pattern` cells with specific content would be content-authoring driven by pattern-finding, which the firewall §2 prohibits. The configuration builder that generates the entries is the E0 work itself; the zero-count is the honest state until that surface ships. (`META_v21_1_methodology_firewall.md` §8; the live count is confirmed zero at data v99.)
