...
The canonical data: `data/Map_v34_consolidated.json` (filename stable per `PROJECT_INFRASTRUCTURE.md` convention) — internal version `v36` as of Step 5, 71 nodes, 186 edges, 30 FCs, 97 cross-classification edges, 484 cells (376 annotated `constructive_status: realized` after Step 4; 108 left field-absent), 5 ADE-clique cross-classification edges decorated with `axis_mapping` after Step 5. The `_meta._schema` field reads `"Map_v16_schema.json"` (labeling drift that predated the v16 work; now resolved as of Step 2).
...
3. Tell it which step the work resumes at — e.g., *"Pick up at Step 6 of PREDICTIVE_LAYER_HANDOFF.md, the Bucket 3 (`conjectured-by-pattern`) backfill for the SU(5)-GUT cells."* Step 6 does not yet have a dedicated sketch section in this handoff; the Step 6 chat composes one as part of its PR, mirroring the structure of the Step 3 / Step 5 sketches above.
...
*End of PREDICTIVE_LAYER_HANDOFF.md, v5 (Step 5 shipped). Update after every shipped step.*
