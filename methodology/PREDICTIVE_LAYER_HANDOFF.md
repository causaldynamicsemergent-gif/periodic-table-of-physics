...
The canonical data: `data/Map_v34_consolidated.json` (filename stable per `PROJECT_INFRASTRUCTURE.md` convention) — internal version `v35` as of Step 4, 71 nodes, 186 edges, 30 FCs, 97 cross-classification edges, 484 cells (376 annotated `constructive_status: realized` after Step 4; 108 left field-absent). The `_meta._schema` field reads `"Map_v16_schema.json"` (labeling drift that predated the v16 work; now resolved as of Step 2).
...
- **Step 5 — ADE-clique `axis_mapping` decoration.** Substantive authoring sweep over the five ADE-connected classifications (`modular-tensor-categories`, `ade-lie-algebras`, `ade-modular-invariants`, `ade-quivers`, `ade-du-val`). The highest-leverage Phase A authoring per the spec §1 and the goal doc §5. The 5 Rule 22 warnings the validator currently emits become 0 once this step ships.
