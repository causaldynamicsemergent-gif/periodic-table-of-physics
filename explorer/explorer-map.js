...
function showTileTooltip(e, fcId) {
  ...
  if (fc.cells && fc.cells.length > 0 && fc.cells.length <= MINI_GRID_THRESHOLD) {
    ...
    if (b) {
      ...
      if (parts.length) {
        cellBreakdownHtml = `<br><span style="color:var(--ink-faint);font-size:13.5px">cells: ${esc(parts.join(' · '))}</span>`;
      }
    }
  }
  ...
  const recurHtmlTT = recurTT.has
    ? `<br><span style="color:var(--accent);font-size:13.5px">⇄ ${esc(recurTT.title)}</span>`
    : '';
