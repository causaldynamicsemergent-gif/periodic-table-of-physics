...
function showEdgeTooltip(ev, id) {
  ...
  t.innerHTML = `
    ...
    ${bits.length ? `<div style="font-family:'JetBrains Mono',monospace;font-size:12.5px;color:var(--ink-mute);margin-bottom:${short ? '4px' : '0'}">${esc(bits.join(' · '))}</div>` : ''}
    ${short ? `<div style="font-size:14px;line-height:1.4;color:var(--ink-soft)">${esc(short)}</div>` : ''}
    <div style="font-size:12.5px;color:var(--ink-faint);margin-top:4px">click to light · click again to switch off</div>
  `;
