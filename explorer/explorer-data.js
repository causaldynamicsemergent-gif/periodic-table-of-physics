...
var state = {
  ...
  activePanel: 'about',         // phenomena | legend | about | education | research | search | spotlight | fc | discourse | glossary | edge
...
function formatPara(s) {
  ...
  return escaped.replace(/\x00MATH(\d+)\x01/g, (_, n) => {
    ...
    if (typeof katex === 'undefined' || !katex || typeof katex.renderToString !== 'function') {
      ...
      return `<${wrap} class="math-unrendered">${esc((seg.displayMode ? '$$' : '$') + seg.latex + (seg.displayMode ? '$$' : '$'))}</${wrap}>`;
    }
    try {
      ...
      return `<code class="math-unrendered" title="${esc(String(e && e.message || e))}">${esc(seg.latex)}</code>`;
    }
  });
}
