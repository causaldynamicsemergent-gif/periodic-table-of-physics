...
function switchSidebarPanel(panel) {
  // Special-case: 'browse-classifications' opens the original classifications dropdown
  if (panel === 'browse-classifications') {
    browseMode = 'classifications';
    buildBrowseMenu('');
...
// =============================================================
//   Browse dropdown — 11-tab navigator (Update B)
//   Two sections: MAP (5 tabs as in Update A) and CATALOGUES
//   (6 tabs: existing All-30-classifications + 5 new discourse-type
//   catalogues for architectures / frontiers / totalities /
//   regime-content / programs). The Classifications sub-mode (with
//   type-ahead search) is unchanged from A.1.
// =============================================================
var browseMode = 'tabs'; // 'tabs' | 'classifications'
function buildBrowseMenu(filter) {
  const menu = document.getElementById('browse-menu');
  if (browseMode === 'tabs') {
    const dc = (DATA.discourse_by_type) || {};
    const ct = type => ((dc[type] && dc[type].length) || 0);
    const sections = [
      {
        head: 'Map',
        tabs: [
      ...
      },
      {
        head: 'Catalogues',
        tabs: [
          { id: 'browse-classifications', icon: '☰', title: 'All ' + FCS.length + ' classifications',        desc: 'Searchable list of formal classifications, grouped by sector.' },
          { id: 'browse-architectures',   icon: '⌬', title: 'Architectures · ' + ct('architecture'),         desc: 'Established + candidate-foundational programs that organize how physics gets done.' },
          { id: 'browse-frontiers',       icon: '✕', title: 'Open frontiers · ' + ct('open-frontier'),       desc: 'Grouped by the structural reason the frontier remains open.' },
          { id: 'browse-totalities',      icon: '◇', title: 'Totality approaches · ' + ct('totality-approach'), desc: 'Whole-system organizing principles spanning architectures.' },
          { id: 'browse-regime-content',  icon: '▣', title: 'Regime content · ' + ct('regime-content'),      desc: 'Empirically-confirmed regimes the architectures produce.' },
          { id: 'browse-programs',        icon: '⚙', title: 'Experimental programs · ' + ct('experimental-program'), desc: 'Collaborations, accelerators, surveys, data-curation efforts.' },
        ],
      },
    ];
    ...
    for (const section of sections) {
      html += `<div class="menu-section-head">${esc(section.head)}</div>`;
      html += section.tabs.map(t => {
        const active = (state.activePanel === t.id && t.id !== 'browse-classifications');
    ...
    menu.querySelectorAll('[data-tab]').forEach(el => {
      el.addEventListener('click', () => {
        ...
        if (tab === 'browse-classifications') {
          browseMode = 'classifications';
          buildBrowseMenu('');
          return;
        }
        switchSidebarPanel(tab);
      });
    });
    return;
  }

  // Classifications list (the original Browse behavior — unchanged)
  ...
  const back = `<div class="menu-tab" data-back="1" style="cursor:pointer;font-weight:600;font-size:11.5px"><span class="tab-icon">‹</span>back to menu</div>`;
  const searchBar = `<div class="menu-search"><input type="search" id="browse-search" placeholder="filter by symbol or name…" value="${esc(filter || '')}" autocomplete="off"></div>`;

  if (orderedKeys.length === 0) {
    menu.innerHTML = back + searchBar + `<div style="padding:14px;font-size:12px;color:var(--ink-mute);font-style:italic">no matches</div>`;
  } else {
    const html = orderedKeys.map(sector => {
      const items = groups[sector].map(fc => `
        <div class="menu-item" data-fc="${esc(fc.id)}">
          <span class="menu-symbol" style="color:var(--${fc.category === 'structural' ? 'struct' : fc.category === 'hybrid' ? 'hybrid' : 'phen'})">${esc(fc.symbol)}</span>
          <span class="menu-name">${esc(fc.label)}</span>
          <span class="menu-counts">${fc.cell_count}c · ${fc.prediction_count}p</span>
        </div>
      `).join('');
      return `<div class="menu-group">
        <div class="menu-group-head">${esc(sector)} <span style="opacity:0.6">(${groups[sector].length})</span></div>
        ${items}
      </div>`;
    }).join('');
    menu.innerHTML = back + searchBar + html;
  }
  ...
  const backEl = menu.querySelector('[data-back]');
  if (backEl) backEl.addEventListener('click', () => { browseMode = 'tabs'; buildBrowseMenu(''); });
  const search = menu.querySelector('#browse-search');
  if (search) {
    search.addEventListener('input', () => buildBrowseMenu(search.value));
    if (filter) {
      const pos = search.value.length;
      search.focus();
      try { search.setSelectionRange(pos, pos); } catch (e) {}
    }
  }
}
