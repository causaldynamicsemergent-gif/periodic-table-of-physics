...
function renderSidebarDefault() {
  ...
  inner.querySelectorAll('[data-quick]').forEach(b => {
    b.addEventListener('click', () => {
      ...
      if (v === 'falsified') { state.highlight = 'falsified'; }
      else if (v === 'confirmed') { state.highlight = 'confirmed'; }
...
function wirePanelJumps(root) {
  ...
  root.querySelectorAll('[data-spotlight]').forEach(el => {
    el.addEventListener('click', () => {
      state.highlight = el.dataset.spotlight;
      syncToolbarChips();
      writeHash();
      renderMap();
      showToast(`Spotlight: ${state.highlight}`);
    });
  });
...
// =============================================================
//   Browse dropdown — six-tab navigator (Update A)
//   When a tab is in "classifications" mode, the original list +
//   type-ahead search appears.
// =============================================================
...
function buildBrowseMenu(filter) {
  ...
  if (browseMode === 'tabs') {
    const tabs = [
      ...
      { id: 'browse-classifications', icon: '☰', title: 'All 30 classifications', desc: 'Searchable list, grouped by sector.' },
    ];
  ...
  // Classifications list (the original Browse behavior)
